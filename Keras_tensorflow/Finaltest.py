...
def get_range(img, x, y, kernelsize=15):
    F = 365.337
    R = 245.0
    minus = int((kernelsize - 1) / 2)
    plus = int((kernelsize / 2) + 1)
    box_Range = img[int(x - minus):int(x + plus), int(y - minus):int(y + plus)]
...
if __name__ == '__main__':
    ...
    #pesi = 'weights\weights.099-0.00737.hdf5'
    # model
        ...
        for index,i in enumerate(pred):
            ...
            if gt_head:
                ...
                for el in gt_coord:
                    ...
                    if show:
                        cv2.rectangle(gt, (int(el[0] - r / 2), int(el[1] - r / 2)), (int(el[0] + r / 2), int(el[1] + r / 2)), (255, 0, 0),1)
